...
function evaluate_grid(env0_name, env1_name, taper, loss_fn, loss_scale, lr)
    ...
    for ei = 1:numel(elevs)
        for ai = 1:numel(azims)
            elev_deg = elevs(ei);
            azim_deg = azims(ai);
            [res, w] = run_optimization(env0_name, env1_name, taper, elev_deg, azim_deg, loss_fn, loss_scale, lr, false);
            results{ei, ai} = res;
        end
    end
    ...
    nmse_diff = nmses_opt - nmses_env1; % match Python
...
function [res, w] = run_optimization(env0_name, env1_name, taper, elev_deg, azim_deg, loss_fn, loss_scale, lr, do_plot)
    ...
    [power_db_opt, w] = optimize(w, aeps_env1, power_env0, loss_fn, loss_scale, lr);
    ...
    if do_plot
        ...
        % Additional weights visualization figure (magnitude and phase)
        ...
        colormap(ax2, 'twilight'); % cyclic colormap similar to Python
        caxis(ax2, [-180, 180]);
...
function draw_pattern_row(tl, row_idx, power_db, label)
    ...
    imagesc(axA, theta_deg, phi_deg, power_db.'); set(axA,'YDir','normal');
    xlabel(axA,'\theta (deg)'); ylabel(axA,'\phi (deg)'); title(axA, [label ' | 2D']);
    ...
    axis(axC,'vis3d'); title(axC, [label ' | 3D']);
end
