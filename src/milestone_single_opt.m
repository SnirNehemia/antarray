...
switch mode
    case 'test_single'
        ...
        for i=1:4
            ...
            [res, w] = run_optimization(env0_name, env1_name, taper, elev_deg, azim_deg, loss_fn, loss_scale, lr, true);
        end
    case 'evaluate_grid'
        milestone()
end
...
function draw_pattern_row(tl, row_idx, power_db, label)
    ...
    ylabel(axA,'\theta (deg)'); xlabel(axA,'\phi (deg)'); title(axA, [label ' | 2D']);
